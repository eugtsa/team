...
def iterate_matches(matches_filename, iterate_every=0, total_jobs=1):
    with bz2.BZ2File(matches_filename) as f:
        for n, line in tqdm(enumerate(f),position=iterate_every,leave=False):
            if n%total_jobs == iterate_every:
                match = json.loads(line)
                yield match

def create_table_parallel(matches_filename, time_point, n_jobs=1):
    if n_jobs>1:
        my_pool = Pool(n_jobs)
        all_args = [(matches_filename, time_point, job_num, n_jobs) for job_num in range(n_jobs)]
        return pandas.concat(my_pool.map(create_one_table,all_args))

    return create_one_table((matches_filename, time_point, 0,1))

def create_one_table(args):
    matches_filename, time_point, jobnum, total_jobs = args

    df = {}
    fields = None
    for match in iterate_matches(matches_filename, iterate_every=jobnum, total_jobs=total_jobs):
...
if __name__ == '__main__':
    ...
    features_table = create_table_parallel(args.input_matches, args.time, args.n_jobs)
    features_table.to_csv(args.output_csv+'.bz2', compression='bz2')
